...
describe('Pagination', () => {
    ...
    test('can set the `tab` role to each item in pagination.', () => {
        (0, test_1.init)();
        const items = Array.from(document.getElementsByClassName(classes_1.CLASS_PAGINATION_PAGE));
        items.forEach(item => {
    ...
    describe.each([
        ...
        test('can loop focus by arrow keys.', () => {
            ...
            (0, test_1.fire)(items[0], 'keydown', { key: prevKey });
            expect(items[end] === document.activeElement).toBe(true);
            expect(splide.index).toBe(end);
            (0, test_1.fire)(items[end], 'keydown', { key: nextKey });
        ...
        test('can focus the first slide by  and the last slide by End.', () => {
            ...
            const items = document.getElementsByClassName(classes_1.CLASS_PAGINATION_PAGE);
            const end = splide.length - 1;
            (0, test_1.fire)(items[0], 'keydown', { key: 'End' });
            expect(items[end] === document.activeElement).toBe(true);
            expect(splide.index).toBe(end);
            (0, test_1.fire)(items[end], 'keydown', { key: 'Home' });
            expect(items[0] === document.activeElement).toBe(true);
            expect(splide.index).toBe(0);
        });
        test('can set proper orientation according to the direction.', () => {
            (0, test_1.init)({ speed: 0, direction: direction, height: 300 });
            const pagination = document.querySelector(`.${classes_1.CLASS_PAGINATION}`);
    ...
    test('should not activate keyboard shortcuts if `paginationKeyboard` option is disabled.', () => {
        const splide = (0, test_1.init)({ paginationKeyboard: false, speed: 0 });
        const items = document.getElementsByClassName(classes_1.CLASS_PAGINATION_PAGE);
        (0, test_1.fire)(items[0], 'keydown', { key: 'ArrowRight' });
