...
describe('Slide', () => {
    ...
    test('can toggle the `is-active` class.', () => {
        ...
        expect(Slide0.slide.classList.contains(classes_1.CLASS_ACTIVE)).toBe(true);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_ACTIVE)).toBe(false);
        splide.go(1);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_ACTIVE)).toBe(false);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_ACTIVE)).toBe(true);
    });
    test('can emit an event when the slide becomes active.', () => {
        ...
        splide.on(events_1.EVENT_ACTIVE, callback);
        splide.go(1);
    ...
    test('can emit an event when the slide becomes inactive.', () => {
        ...
        const Slide2 = Slides.getAt(2);
        const callback = jest.fn();
        splide.on(events_1.EVENT_INACTIVE, callback);
        splide.on(events_1.EVENT_INACTIVE, callback);
        splide.go(1);
    ...
    test('can toggle the `is-visible` class and the `aria-hidden` attribute.', () => {
        const splide = (0, test_1.init)({ speed: 0, perPage: 2 });
        const { Slides } = splide.Components;
        const Slide0 = Slides.getAt(0);
        const Slide1 = Slides.getAt(1);
        const Slide2 = Slides.getAt(2);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(true);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(true);
        expect(Slide2.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(false);
        ...
        splide.go(1);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(false);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(true);
        expect(Slide2.slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(true);
    ...
    test('should not update aria-hidden on move even if `updateOnMove` is enabled.', () => {
        const splide = (0, test_1.init)({ speed: 100, updateOnMove: true });
        const { Slides } = splide.Components;
    ...
    test('can emit an event when the slide becomes visible.', () => {
        ...
        const callback = jest.fn();
        const visibleSlides = [];
        splide.on(events_1.EVENT_VISIBLE, Slide => {
            callback();
    ...
    test('can toggle `tabindex` by visibility if `slideFocus` is enabled.', () => {
        const splide = (0, test_1.init)({ speed: 0, slideFocus: true });
        const { Slides } = splide.Components;
        const Slide0 = Slides.getAt(0);
        const Slide1 = Slides.getAt(1);
    ...
    test('can disable focus of focusable descendants when the slide gets hidden.', () => {
        const splide = (0, test_1.init)({ speed: 0 }, { mount: false });
        const slide0 = document.querySelector(`.${classes_1.CLASS_SLIDE}`);
    ...
    test('can emit an event when the slide gets hidden.', () => {
        ...
        const callback = jest.fn();
        const hiddenSlides = [];
        splide.on(events_1.EVENT_HIDDEN, Slide => {
            callback();
    ...
    test('can toggle the `is-prev` and `is-next` classes.', () => {
        ...
        const Slide1 = Slides.getAt(1);
        const Slide2 = Slides.getAt(2);
        expect(Clone1.slide.classList.contains(classes_1.CLASS_PREV)).toBe(true);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_PREV)).toBe(false);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_NEXT)).toBe(false);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_NEXT)).toBe(true);
        splide.go(1);
        expect(Clone1.slide.classList.contains(classes_1.CLASS_PREV)).toBe(false);
        expect(Slide0.slide.classList.contains(classes_1.CLASS_PREV)).toBe(true);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_PREV)).toBe(false);
        expect(Slide1.slide.classList.contains(classes_1.CLASS_NEXT)).toBe(false);
        expect(Slide2.slide.classList.contains(classes_1.CLASS_NEXT)).toBe(true);
    });
    ...
    test('should assign the tabpanel role if the pagination option is enabled.', () => {
        const splide = (0, test_1.init)({ pagination: true });
    ...
    test('should assign group tab role with `aria-roledescription` if the pagination option is disabled.', () => {
        const splide = (0, test_1.init)({ pagination: false });
        splide.Components.Slides.forEach(({ slide }) => {
    ...
    test('can assign and update role/aria attributes for navigation.', () => {
        const splide = (0, test_1.init)({ speed: 0, isNavigation: true, pagination: false });
        const { Slides } = splide.Components;
        const Slide0 = Slides.getAt(0);
        const Slide1 = Slides.getAt(1);
        expect(Slide0.slide.getAttribute('aria-current')).toBe('true');
        expect(Slide0.slide.getAttribute('aria-label')).toBe((0, utils_1.format)(splide.options.i18n.slideX, 1));
        expect(Slide1.slide.getAttribute('aria-current')).toBeNull();
        expect(Slide1.slide.getAttribute('aria-label')).toBe((0, utils_1.format)(splide.options.i18n.slideX, 2));
        splide.go(1);
    ...
    test('can emit the `click` event on click.', done => {
        ...
        splide.on(events_1.EVENT_CLICK, Slide => {
            expect(Slide.slide).toBe(slide);
            done();
        });
        (0, test_1.fire)(slide, 'click');
    });
    test('can emit the `slide:keydown` event on keydown.', done => {
        const splide = (0, test_1.init)({ speed: 0, isNavigation: true });
        const { Slides } = splide.Components;
        const { slide } = Slides.getAt(2);
        splide.on(events_1.EVENT_SLIDE_KEYDOWN, (Slide, e) => {
            expect(Slide.slide).toBe(slide);
            expect(e.key).toBe('Enter');
            done();
        });
        (0, test_1.keydown)('Enter', slide);
    });
    test('should remove status classes and added attributes.', () => {
        ...
        const { slide } = Slides.getAt(0);
        splide.destroy();
        expect(slide.classList.contains(classes_1.CLASS_ACTIVE)).toBe(false);
        expect(slide.classList.contains(classes_1.CLASS_VISIBLE)).toBe(false);
    ...
    test('can notify the active slide of other components on initialization.', () => {
        const splide = (0, test_1.init)({ start: 1 }, { mount: false });
        const callback = jest.fn();
        const component = (Splide) => {
            return {
                setup() {
                    Splide.on(events_1.EVENT_ACTIVE, Slide => {
    ...
    test('can check some slide is within the specified distance.', () => {
        const splide = (0, test_1.init)({ perPage: 2, type: 'loop' });
        const { Slides } = splide.Components;
        const Slide0 = Slides.getAt(0);
        const Slide1 = Slides.getAt(1);
        const Slide2 = Slides.getAt(2);
