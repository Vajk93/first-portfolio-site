...
describe('Move', () => {
    ...
    test('can loop the slider if it exceeds bounds.', () => {
        const width = 200;
        const splide = (0, test_1.init)({ type: 'loop', width, height: 100 });
        const { Move } = splide.Components;
        const { list } = splide.Components.Elements;
        // All clones do not have dimensions in this jest env. Forcibly sets them.
        (0, test_1.setSlidesRect)(splide.Components.Slides.get().map(Slide => Slide.slide), list, width, 1);
        Move.jump(0);
    ...
    test('can check the position exceeds bounds or not.', () => {
        const width = 200;
        const splide = (0, test_1.init)({ width, height: 100 });
