...
describe('Sync#sync()', () => {
    test('can sync Splide sliders.', () => {
        ...
        const secondary = (0, test_1.init)({ speed: 0 }, { id: 'secondary', insertHtml: true, mount: false });
    ...
    test('can sync multiple Splide sliders.', () => {
        ...
        Array.from({ length: 5 }).forEach((item, index) => {
            const secondary = (0, test_1.init)({ speed: 0 }, { id: `secondary-${index}`, insertHtml: true, mount: false });
    ...
    test('should prevent recursive calls of `move` event handlers.', () => {
        ...
        primary.sync(secondary).mount();
        secondary.mount();
        primary.on(events_1.EVENT_MOVE, callback);
        primary.go(1);
